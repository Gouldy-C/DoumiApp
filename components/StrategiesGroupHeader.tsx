...
const StrategiesGroupHeader = ({groupIndex, image, variant} : {groupIndex: number, image: ImageSourcePropType, variant: 'next' | 'back'}) => {
  if (variant === 'back') {
    return (
      <View style={{width: '90%', flexDirection: 'row', alignSelf: 'center', borderRadius: 24,  gap: 2, backgroundColor: getColor(Number(groupIndex)), overflow:"hidden", justifyContent: 'space-between', marginBottom: 20}}>
  ...
  if (variant === 'next') {
    ...
    return(
      <View style={{width: '90%', flexDirection: 'row-reverse', alignSelf: 'center', borderRadius: 24,  gap: 2, backgroundColor: getColor(Number(groupIndex) + 1), overflow:"hidden", justifyContent: 'space-between', marginTop: 30}}>
        <View style={{paddingHorizontal: 8, paddingVertical: 4, flex: 1, flexDirection: 'column-reverse'}}>
          ...
          <Text style={{color:'white', fontSize: 30, flex: 1, textAlign: 'center', paddingHorizontal: 2, textAlignVertical: 'center'}}>{strategyCatagories[groupIndex].title}</Text>
        </View>
        <View style={{
            height: 157,
            width: 157,
