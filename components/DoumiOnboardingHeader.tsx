...
const DoumiOnboardingHeader = ({textValue, textSize}: {textValue: string, textSize: 'small' | 'large'}) => {
  return (
    <View style={{justifyContent: 'center', alignItems: 'center', gap: 16, marginBottom: 15}}>
      ...
      <Text style={{fontSize: textSize === 'small' ? 20 : 32, textAlign: 'center', textAlignVertical: 'center', color: '#5049A4', paddingHorizontal:textSize === 'small' ? 35 : 10, fontWeight: textSize === 'small' ? '400' : '600', marginVertical: textSize === 'small' ? 25 : 10 }}>{textValue}</Text>
    </View>
  )
}
